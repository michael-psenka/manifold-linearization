...
class GeneratorMNIST(dcgan_base.DCGANBaseGenerator):
    ...
    def __init__(self, nz=128, ngf=64, bottom_width=4, **kwargs):
        ...
        #     nn.ReLU(),
        #     nn.ConvTranspose2d(ngf * 2* 7 * 7, ngf, 4, stride=2, padding=1, bias=False),
        #     nn.BatchNorm2d(ngf),
        #     nn.ReLU(),
...
class DiscriminatorMNIST(dcgan_base.DCGANBaseDiscriminator):
    ...
    def __init__(self, nz=128, ndf=64, **kwargs):
        ...
        #     nn.BatchNorm2d(ndf),
        #     nn.LeakyReLU(0.1),
        #     nn.Conv2d(ndf, 2*ndf*7*7, 3, stride=2, padding=1, bias=False),
        #     nn.BatchNorm2d(2*ndf*7*7),
        #     nn.LeakyReLU(0.1),
        ...
        #     nn.Conv2d(64, 128*7*7, 3, stride=2, padding=1, bias=False),
        #     nn.BatchNorm2d(128),
        #     nn.LeakyReLU(0.1),
        # )
...
class MCRTrainer(MUltiGPUTrainer):
    ...
    def __init__(self,
        ...
        self.mcr_gan_loss = MCRGANloss(gam1=1., gam2=1., gam3=1., eps=0.5, numclasses=num_class, mode=mode, rho=(1.0, 1.0))
...
idx = torch.tensor(dataset.targets) == 2
dataset = torch.utils.data.dataset.Subset(dataset, np.where(idx==1)[0])
dataloader = torch.utils.data.DataLoader(dataset, batch_size=2048, shuffle=True, num_workers=4)



# Define models and optimizers
netD, netG = get_models('mnist', device)

optD = optim.Adam(netD.parameters(), lr=0.00015, betas=(0.5, 0.999))
...
trainer = MCRTrainer(
    ...
    log_dir='models/CTRL/logs/mnist_LDR_multi',
    device=device,
    num_class=1,
    mode=0,
)
...
with open("models/CTRL/results/ctrl_training_time.txt", "w") as text_file:
    text_file.write("CTRL Training Time: %.3f" % (end_time - start_time))
torch.save(netD, "./models/CTRL/results/disc.torch")
torch.save(netG, "./models/CTRL/results/gen.torch")